...
**The actual primary/recommended result is `scripts/ensemble.py`** (`docs/documentation.txt` Part 14, needs `pip install xgboost`): a soft-voting ensemble of three structurally dissimilar members (XGBoost on `gbt_baseline.py`'s 55 aggregated features / mixed pool; XGBoost on `features.py`'s 198 physics features / real-CISS-only; ExtraTrees on the 198 physics features), scoring **53.9% +/- 2.2% accuracy, macro-F1 0.436** on real CISS crashes. It rebuilds its own tensors from the raw CSV (~8 min), so it has no prerequisites.

It is evaluated with **repeated** CV (3 seeds x 5 folds) and that matters: per-fold accuracy varies by 2-3 points, so single-pass 5-fold CV cannot resolve a 1-2 point change. Two changes measured +0.7pp and +2.2pp on one seed and -0.18pp and +0.57pp over three (`docs/documentation.txt` Part 14.3). **Never compare configurations on a single seed in this repo** — use `ensemble.py`'s default 3 seeds. `--seeds 42` is for quick smoke runs only.

The previous primary result, `scripts/cross_validate.py` (`docs/documentation.txt` Part 13), is kept as the single-pass-CV comparison: a class-weighted XGBoost model on aggregated per-event features (not raw sequences — see `scripts/gbt_baseline.py`), evaluated with 5-fold CV over all 1,520 real CISS crashes so every one is tested exactly once. It beats every deep model and reports mean±std instead of one split's number. Run it after `build_sequences.py --split-mode mixed`; it rebuilds its own tensors from the raw CSV internally (~1-2 min) so it doesn't depend on train.py having been run first.

The dataset is now **12,113 events / 1.04M rows** across five sources (SynSHRP2 5,701, CTDB 3,442, CISS 1,520, BeamNG 1,450). CTDB — real staged NHTSA crash tests — was added in Phase 2 via `scripts/fetch_crash_test_index.py` → `download_crash_test_db.py` → `integrate_crash_test_db.py`; its raw downloads (16,816 files) are git-ignored since the processed output is already merged in.

**The merged dataset is committed GZIPPED** as `Dataset/data/training_dataset_final.csv.gz` (~17 MB) — the plain CSV is ~120 MB, over GitHub's 100 MB per-file limit, and is git-ignored. `config.py`'s `DATA_CSV` prefers the plain file when present and falls back to the `.gz`; pandas decompresses transparently, so no calling code changes. On a fresh clone everything works from the `.gz` directly.
